...
import { ArrowRight, Menu, X, Share2, Users, HardDrive, Zap, CheckCircle2, HelpCircle, BookOpen, Award, FileText, Mail, ChevronDown } from 'lucide-react';
...
const focusAreas = [
  { title: "SharePoint Online", description: "Information architecture, hub strategy, publishing patterns, site owner readiness, and content quality standards." },
  { title: "Microsoft Teams", description: "Team lifecycle, channel design, collaboration norms, governance, and working agreements that reduce sprawl." },
  { title: "OneDrive", description: "Migration readiness, sharing guidance, ownership decisions, and practical habits for confident file management." },
  { title: "Power Platform", description: "Citizen-maker guardrails, intake models, environment strategy, and enablement paths that keep innovation responsible." }
];
...
const outcomeCards = [
  { title: "Executive visibility", description: "A clear adoption scorecard links rollout progress, readiness, governance, and user confidence." },
  { title: "Cleaner collaboration", description: "Teams, channels, SharePoint sites, and OneDrive sharing all follow a practical operating model." },
  { title: "Stronger ownership", description: "Site owners and champions understand what they own, how to maintain it, and when to escalate." },
  { title: "Sustainable change", description: "Training, communications, support, and improvement loops continue after launch day." }
];
...
const icons = [Share2, Users, HardDrive, Zap];
...
function Header() {
  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 backdrop-blur-md shadow-sm border-b border-border' : 'bg-transparent'}`}
    >
      <div className="container flex items-center justify-between h-20">
        <motion.a href="#" className="flex items-center gap-3 group" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-primary to-cyan flex items-center justify-center text-white font-bold text-lg">A</div>
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <motion.a key={item.href} href={item.href} className="text-sm font-medium text-foreground/70 hover:text-foreground transition-colors" whileHover={{ scale: 1.05 }}>
              {item.label}
            </motion.a>
          ))}
        </nav>
        ...
        <div className="hidden md:flex items-center gap-4">
          <motion.a href="#contact" className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-blue-primary to-cyan hover:shadow-lg hover:scale-105 transition-all duration-200" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            Get Started
          </motion.a>
        </div>
      ...
      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: isOpen ? 1 : 0, height: isOpen ? 'auto' : 0 }} transition={{ duration: 0.3 }} className="md:hidden overflow-hidden border-t border-border bg-white">
        <nav className="container py-4 flex flex-col gap-3">
          {navItems.map((item) => (
            <a key={item.href} href={item.href} className="px-4 py-2 rounded-lg text-sm font-medium text-foreground/70 hover:bg-muted hover:text-foreground transition-all" onClick={() => setIsOpen(false)}>
              {item.label}
            </a>
          ))}
          <a href="#contact" className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-blue-primary to-cyan hover:shadow-lg hover:scale-105 transition-all duration-200 text-sm mt-2" onClick={() => setIsOpen(false)}>
            Get Started
          </a>
        </nav>
      </motion.div>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-20">
        ...
        <section className="relative w-full min-h-screen flex items-center justify-center overflow-hidden" style={{
          ...
          <motion.div className="container max-w-5xl z-10 relative" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ staggerChildren: 0.15, delayChildren: 0.3 }}>
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: "easeOut" }} className="text-center mb-8">
              <span className="inline-block px-5 py-2.5 rounded-full bg-white/15 backdrop-blur-md text-white font-semibold text-sm border border-white/20 shadow-lg">{siteConfig.tagline}</span>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.3, ease: "easeOut" }} className="flex flex-col sm:flex-row gap-6 justify-center items-center mb-16">
              <motion.a href="#contact" className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-lg font-bold text-white bg-gradient-to-r from-blue-primary to-cyan hover:shadow-2xl hover:scale-110 transition-all duration-300 group text-lg" whileHover={{ scale: 1.1, boxShadow: '0 20px 40px rgba(0, 82, 204, 0.4)' }} whileTap={{ scale: 0.95 }}>
                Start the roadmap
                <ArrowRight className="w-5 h-5 group-hover:translate-x-2 transition-transform" />
              </motion.a>
              <motion.a href="#diagnostic" className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-lg font-bold text-white bg-white/20 backdrop-blur-md border border-white/30 hover:bg-white/30 transition-all duration-300 text-lg" whileHover={{ scale: 1.1, backgroundColor: 'rgba(255, 255, 255, 0.3)' }} whileTap={{ scale: 0.95 }}>
                Check readiness
              </motion.a>
            </motion.div>
          ...
          <div className="container">
            ...
            <motion.div className="grid md:grid-cols-2 gap-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.1 }} viewport={{ once: true }}>
              {focusAreas.map((area, index) => {
                ...
                return (
                  <motion.div key={index} className="p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 group cursor-pointer" whileHover={{ y: -4, boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
                    <div className="flex gap-4">
                      <div className="flex-shrink-0">
                        <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-blue-light to-cyan-light flex items-center justify-center group-hover:from-blue-light group-hover:to-cyan-light transition-all">
                          <Icon className="w-6 h-6 text-blue-primary" />
                        </div>
                      </div>
                      <div className="flex-1">
                        <h3 className="font-semibold text-lg text-foreground mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>{area.title}</h3>
          ...
          <div className="container">
            ...
            <motion.div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.05 }} viewport={{ once: true }}>
              {services.map((service, index) => (
                <motion.div key={index} className="p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 group relative overflow-hidden" whileHover={{ y: -6 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
                  <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-blue-light to-cyan-light rounded-full -mr-10 -mt-10 group-hover:scale-150 transition-transform duration-300" />
                  <div className="relative z-10">
                    <div className="text-4xl font-bold text-blue-primary/20 mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>{service.number}</div>
        ...
        {/* Engagements */}
        <section id="engagements" className="py-20 bg-gradient-to-b from-white to-muted/30">
          <div className="container">
            <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            ...
            <motion.div className="grid md:grid-cols-3 gap-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.1 }} viewport={{ once: true }}>
              {engagementModels.map((model, index) => (
                <motion.div key={index} className={`p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden ${model.featured ? 'md:scale-105 md:shadow-xl border-blue/50' : ''}`} whileHover={{ y: -6 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
                  {model.featured && <div className="absolute top-0 right-0 px-4 py-1 bg-gradient-to-r from-blue-primary to-cyan text-white text-xs font-semibold rounded-bl-lg">Popular</div>}
                  <div>
                    <div className="flex items-start justify-between mb-4">
                      <h3 className="font-semibold text-xl text-foreground" style={{ fontFamily: "'Playfair Display', serif" }}>{model.title}</h3>
                    </div>
                    <div className="inline-block px-3 py-1 rounded-full bg-blue-light text-blue-primary text-xs font-semibold mb-4">{model.duration}</div>
                    ...
                    <motion.button className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-blue-primary to-cyan hover:shadow-lg hover:scale-105 transition-all duration-200 w-full mt-6 text-sm" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                      Learn More
                    </motion.button>
                  </div>
                </motion.div>
                ...
                {journey.map((phase, index) => (
                  <motion.div key={index} className={`flex gap-8 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }}>
                    <div className="hidden md:flex flex-col items-center flex-shrink-0">
                      <motion.div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-primary to-cyan border-4 border-white shadow-lg" whileHover={{ scale: 1.3 }} />
                    </div>
                    <motion.div className="flex-1 p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300" whileHover={{ y: -4 }}>
                      <div className="flex items-start gap-4">
                        <div className="flex-shrink-0 md:hidden">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-primary to-cyan flex items-center justify-center text-white font-bold text-sm">{phase.number}</div>
                        </div>
                        <div className="flex-1">
        ...
        {/* Diagnostic */}
        <section id="diagnostic" className="py-20 bg-gradient-to-b from-white to-muted/30">
          <div className="container">
            <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            ...
            <motion.div className="grid md:grid-cols-2 gap-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.1 }} viewport={{ once: true }}>
              {diagnosticSignals.map((signal, index) => (
                <motion.div key={index} className="p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 group" whileHover={{ y: -4 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
                  <div className="flex gap-4">
                    <div className="flex-shrink-0">
                      <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-orange-light to-orange-light flex items-center justify-center group-hover:from-orange-light group-hover:to-orange-light transition-all">
            ...
            <motion.div className="mt-12 p-6 rounded-xl border border-blue-light bg-gradient-to-r from-blue-light to-cyan-light" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
              <div className="text-center">
                ...
                <p className="text-foreground/60 mb-6">Let's evaluate your organization's Microsoft 365 readiness and create a tailored roadmap.</p>
                <motion.a href="#contact" className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-blue-primary to-cyan hover:shadow-lg hover:scale-105 transition-all duration-200" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          ...
          <div className="container">
            ...
            <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-4" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.05 }} viewport={{ once: true }}>
              {learningItems.map((item, index) => (
                <motion.div key={index} className="p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 text-center group cursor-pointer" whileHover={{ y: -4, scale: 1.05 }} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }}>
                  <div className="flex flex-col items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-blue-light to-cyan-light flex items-center justify-center group-hover:from-blue-light group-hover:to-cyan-light transition-all">
        ...
        {/* Outcomes */}
        <section id="outcomes" className="py-20 bg-gradient-to-b from-white to-muted/30">
          <div className="container">
                ...
                {outcomeCards.map((outcome, index) => (
                  <motion.div key={index} className="p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 group" whileHover={{ y: -4 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
                    <div className="flex gap-3 mb-4">
                      <Award className="w-6 h-6 text-green flex-shrink-0 mt-1" />
                    </div>
            ...
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
              ...
              <motion.div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.1 }} viewport={{ once: true }}>
                {deliverables.map((deliverable, index) => (
                  <motion.div key={index} className="p-6 rounded-xl border border-blue-light bg-gradient-to-br from-blue-light to-cyan-light shadow-sm hover:shadow-md transition-all duration-300 group" whileHover={{ y: -4 }} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
                    <div className="flex gap-3 mb-4">
                      <FileText className="w-6 h-6 text-blue-primary flex-shrink-0 mt-1" />
                    </div>
        ...
        <section className="py-20 bg-white">
          <div className="container max-w-3xl">
            ...
            <motion.div className="space-y-4" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.05 }} viewport={{ once: true }}>
              {faqs.map((faq, index) => (
                <motion.div key={index} className="p-0 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}>
                  <motion.button onClick={() => setOpenFaqIndex(openFaqIndex === index ? null : index)} className="w-full px-6 py-4 flex items-center justify-between hover:bg-muted/50 transition-colors text-left" whileHover={{ backgroundColor: 'rgba(0,0,0,0.02)' }}>
                  ...
                  <AnimatePresence>
                    {openFaqIndex === index && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="border-t border-border">
            ...
            <motion.div className="max-w-4xl mx-auto" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ staggerChildren: 0.1 }} viewport={{ once: true }}>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="grid md:grid-cols-3 gap-6 mb-12">
                {conversationTopics.map((topic, index) => (
                  <motion.div key={index} className="p-6 rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 text-center" whileHover={{ y: -4 }}>
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row gap-6 justify-center items-center">
                <motion.a href={`mailto:${siteConfig.email}`} className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-semibold text-white bg-gradient-to-r from-blue-primary to-cyan hover:shadow-lg hover:scale-105 transition-all duration-200 group" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  <Mail className="w-5 h-5" />
                  Email us
                  <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                </motion.a>
                <motion.button className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-semibold text-foreground bg-muted hover:bg-muted/80 transition-all duration-200" whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  Schedule a call
                </motion.button>
              </motion.div>
          ...
          <motion.div className="grid md:grid-cols-4 gap-8 mb-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} viewport={{ once: true }}>
            <div>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-primary to-cyan flex items-center justify-center font-bold text-sm">A</div>
